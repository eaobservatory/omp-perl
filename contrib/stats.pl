...
unless (defined $statsmode) {
    ...
    if ($statsmode =~ /^m/i) {
        if (defined $utx) {
            ...
            print STDERR
                "Start monthly stats on UT 1st? (Else start HST 1st = UT 2nd) [y]/n: ";
            chomp($utx = <STDIN>);
            if ($utx =~ /^n/i) {
                $utx = "HST";
            }
...
while ($ut <= $endut) {
    ...
    if ($tel =~ /^j/i) {
        print STDOUT
            "   UT       Projects  Weather  Faults  Other  Extended  Cal    Closed  Unreport  Total  Frate\n"
            if ($loop == 1);
    }
    else {
        print STDOUT
            "   UT       Projects of which EAC  Weather  Faults  Other  Cal    Closed  Total  Frate\n"
            if ($loop == 1);
    }
    ...
    if ($tel =~ /^j/i) {
        ...
        $ref{"available"} -= 12
            if (${yy} * 1e+04 + 1225 >= $iut
            and ${yy} * 1e+04 + 1225 < $iut + $delta
            and $statsmode !~ /^d/i);
...
if ($tel =~ /^j/i) {
    printf STDOUT "The report from $startut covers %d scheduled ", $grand{"days"};
    printf STDOUT "12-hour nights, in total %3d hours:\n\n", 12 * $grand{"days"};
}
else {
    printf STDOUT "The report from $startut covers %d scheduled nights ", $grand{"days"};
