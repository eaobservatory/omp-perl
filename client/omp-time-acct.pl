...
BEGIN {
  # The column names (and possible method names to be called, in order, on given
    ...
    # header strings.
    sub print_header {
        return really_print_header(@header);
    }
}
...
sub make_project_query_2 {
    ...
    $sql or die sprintf <<"NO_SQL", Dumper($projs, $ranges);
...
sub sql_between {
    ...
    for (my $i = 0 ; $i < $size - 1 ; $i += 2) {
        push @out, "$col BETWEEN "
            . join ' AND ', map {$no_quote ? $_ : "'$_'"} @{$in}[$i, $i + 1];
    }
...
sub process_options {
    ...
    if ($opt{'telescope'}) {
        for my $sem (@sem) {
            my %range;
            @range{qw/min max/} = map {$_->ymd} OMP::DateTools->semester_boundary(
                'tel' => $opt{'telescope'}, 'semester' => $sem);

            push @{$opt{'date-range'}}, \%range;
        }
    }

    $opt{'projects'} = [map {$_ ? uc $_ : ()} @ARGV, get_projects(@file)];
...
sub is_non_empty_aref {
    return $_[0] && ref $_[0] && scalar @{ $_[0] };
}
