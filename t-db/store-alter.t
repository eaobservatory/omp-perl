...
die 'Not using test database'
    unless {$db->loginhash()}->{'database'} eq 'devomp';
...
do {
    local $/ = undef;
    my $fh = IO::File->new('store-alter.xml');
    $xml = <$fh>;
    close $fh;
};
...
$msbdb->storeSciProg(SciProg => OMP::SciProg->new(XML => $xml), Force => 1);
