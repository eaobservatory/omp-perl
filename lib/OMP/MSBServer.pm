...
sub historyMSB {
    ...
    OMP::General->log_message(
        "historyMSB: project:" . (defined $project ? $project : "none")
        . ", checksum:" . (defined $checksum ? $checksum : "none")
        . "\n");
...
sub titleMSB {
    ...
    try {
        ...
        my $donedb = OMP::MSBDoneDB->new(DB => $connection);
        $result = $donedb->titleMSB($checksum);

        # Now MSBDB
        unless ($result) {
            my $msbdb = OMP::MSBDB->new(DB => $connection);
...
Return all the MSBs that match the specified XML query
    ...
    $output = OMP::MSBServer->queryMSBdone(
        $xml, {
...
sub queryMSBdone {
    ...
    try {
        ...
        my $q = OMP::MSBDoneQuery->new(XML => $xml);
