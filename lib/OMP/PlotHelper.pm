...
=head1 SYNOPSIS
    ...
    @values_binned = OMP::PlotHelper->bin_up(
        size => 7,
        method => 'sum',
        values => \@values);
...
Number to bin up by, or an array reference of bin sizes.
...
Method to use when binning.  A string that is either
'sum', 'average', or 'max'.

=item values

An array reference whose elements are coordinates where the
...
sub bin_up {
    ...
    if (defined $start_num) {
        ...
        # in the values array
        throw OMP::Error::BadArgs(
            "Argument 'startnum' must be lower than or equal to lowest X value")
...
Bin up values by a number of days (can be fractional).
    ...
    @values_binned = OMP::PlotHelper->bin_up_by_date(
        days => 0.5,
        method => 'sum',
        values => \@values,
        startdate => $date);
...
An array reference whose elements are arrays where the
...
Date to start binning by.  Provide as a C<Time::Piece>
...
sub bin_up_by_date {
    ...
    if ($start_date) {
        ...
        throw OMP::Error::BadArgs(
            "Argument 'startdate' must be a Time::Piece object")
    ...
    for my $val (@values) {
        ...
        throw OMP::Error::BadArgs(
            "Must provide x values as Time::Piece objects")
