...
sub process_project_changes {
    ...
    my $new_state_name = undef;
    if ($new_state xor $project->state) {
        $project->state($new_state ? 1 : 0);

        $new_state_name = $new_state ? 'enabled' : 'disabled';
        push @msg, sprintf 'Project %s.', $new_state_name;
    }
        ...
        $fdb->addComment(
            {
                ...
                msgtype => OMP__FB_MSG_PROJECT_ALTERED(),
                text =>
    ...
    # If the state changed, generate (potentially another) feedback message
    # similar to that which would be generated by the
    # OMP::DB::Project::enableProject / disableProject methods.
    if (defined $new_state_name) {
        $fdb->addComment({
            author => $self->auth->user,
            subject => ('Project ' . $new_state_name),
            text => (sprintf 'Project %s %s',
                $project->projectid, $new_state_name),
            status => OMP__FB_HIDDEN(),
            msgtype => (($new_state_name eq 'enabled')
