        ...
        if (defined $airmass) {
            ...
            $airmass->intersection($airmass_el)
                or throw OMP::Error::FatalError(
                    "The supplied airmass and elevation ranges do not intersect: airmass $airmass and from el: $airmass_el");
        }
        else {
...
Is the MSB still to be observed?

=item allocation

Has the full project allocation been used?
...
Is the source within the zone-of-avoidance?
...
sub sql {
    ...
    if (defined($qhash->{'M.telescope'})) {
        ...
        try {
            my $seeing_eq = OMP::Config->getData(
                'seeing_eq',
    ...
    $constraint_sql .= " AND M.remaining > 0 "
...
sub _post_process_hash {
    ...
    # constrain the query)
    if (! exists $href->{checksum}
            && ! exists $href->{msbid}
            && (! exists $href->{telescope} && ! $href->{_attr}->{projectid}->{full})) {
        throw OMP::Error::MSBMalformedQuery(
            "Please supply a telescope, an explicit unabbreviated project ID or a specific MSB identifier");
    }
    ...
    if (exists $href->{date}) {
        ...
        throw OMP::Error::MSBMalformedQuery(
            "date can not be specified as a range. Only a specific date can be supplied.")
    ...
    for my $key (keys %$href) {
        ...
        if ($key eq "seeing"
            ...
            for my $type (qw/min max/) {
                $href->{"$key$type"} = OMP::Range->new(
                    $outkey{$type} => $href->{$key}->[0]);
            }
            ...
            for my $pid (@{$href->{$key}}) {
                $pid = OMP::General->infer_projectid(%options, projectid => $pid);
            }
    ...
    if (! exists $href->{semester}
            && ! exists $href->{projectid}
            && ! exists $href->{checksum}
            && ! exists $href->{msbid}) {
        # Need the telescope in this case
        throw OMP::Error::MSBMalformedQuery(
            "Automatic semester determination requires a telescope in the query")
        ...
        $href->{semester} = [
            OMP::DateTools->determine_semester(
                tel => $href->{telescope}->[0],
                %options)];
    }
    ...
    $self->_process_elements(
        $href,
        sub {uc(shift)}, [qw/
            projectid telescope semester country name
