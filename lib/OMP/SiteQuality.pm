...
sub from_db {
    ...
    unless (scalar @_) {
        ...
        $range = OMP::Range->new(Min => $_[0], Max => $_[1]);
    }
    else {
...
sub default_range {
    ...
    my $def = OMP::Range->new(
        Min => $OMPRANGES{$cat}->[0],
        Max => $OMPRANGES{$cat}->[1],
        PosDef => (
            defined $OMPRANGES{$cat}->[0] && $OMPRANGES{$cat}->[0] == 0 ? 1 : 0
        ),
    );
...
sub check_posdef {
    ...
    return unless defined $OMPRANGES{$cat}->[0];
...
sub undef_to_default {
    ...
    $range->min($OMPRANGES{$cat}->[0]) unless defined $range->min;
    $range->max($OMPRANGES{$cat}->[1]) unless defined $range->max;
...
sub upgrade_cloud {
    ...
    return OMP::Range->new(Min => $min, Max => $max, posdef => 1);
}
    ...
    if (exists $details{TELESCOPE} and $details{TELESCOPE} eq 'JCMT') {
        if (exists $details{TAU}) {
            ...
            throw OMP::Error::FatalError(
                "CSO TAU supplied but not defined. Unable to determine band")
                unless defined $cso;
            ...
            throw OMP::Error::FatalError(
                "Unable to determine band for JCMT without TAU");
        }
    }
    else {
...

{
    # Specify the bands
    my %bands = (
        0 => OMP::Range->new(Min => 0),
        1 => OMP::Range->new(Min => 0, Max => 0.05),
        2 => OMP::Range->new(Min => 0.05, Max => 0.08),
        3 => OMP::Range->new(Min => 0.08, Max => 0.12),
        4 => OMP::Range->new(Min => 0.12, Max => 0.20),
        5 => OMP::Range->new(Min => 0.20, Max => 0.32),
        6 => OMP::Range->new(Min => 0.32),
        '2*' => OMP::Range->new(Min => 0.05, Max => 0.10),
        '3*' => OMP::Range->new(Min => 0.10, Max => 0.12),
    );
    ...
    sub get_tauband_range {
        ...
        if (defined $tel && $tel eq 'JCMT') {
            ...
            return OMP::Range->new(Min => $min, Max => $max);
        }
        else {
            return $bands{"0"};
        }
