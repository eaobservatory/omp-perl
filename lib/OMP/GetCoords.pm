...
sub get_coords {
    ...
    foreach my $obj (@objects) {
        ...
        if (exists $planets{$objname}) {
            push @coords, Astro::Coords->new(planet => "$objname");
            $nr_planets ++;
            ...
            $coord->telescope(Astro::Telescope->new($args{'telescope'}))
...
Array of science project objects.

=item @objects

Array of hashes with names of selective targets
to find. (optional)
...
Select [ "All" | "Active" | "Completed" ] msbs. Default "All".

=item telescope

If provided, will be added to coord object.
...
sub get_omp_coords {
    # Set up defaults
    my %defaults = (
        'msbmode' => "all", # msbs to retrieve: [all | active | completed]
    ...
    foreach my $sp (@sciprogs) {
        ...
        foreach my $msb (@msbs) {
            ...
            foreach my $obs (@obss) {
                ...
                my $target_info = $target->name . '_' . $target->ra . '_' . $target->dec;
                ...
                # Add telescope information to coords object if provided!
                $target->telescope(Astro::Telescope->new($args{'telescope'}))
...
Array with project ids.

=item @objects

Array with names of selective targets find. (optional)

=item {catalog}

Name of the catalog (can also be given through %args).
...
sub get_catalog_coords {
    ...
    foreach my $line (@catlines) {
        ...
        printf "%16.16s  %12.12s  %12.12s %8.8s\n",
            $target{'name'}, $target{'ra'}, $target{'dec'}, $target{'coordsys'}
            if $debug;
...
Right Ascention and Declination in radians.

=item $equinox, $epoch

'B' Besselian, 'J' Julian, 'D' of date  and
the epoch. The epoch will be '0' for 'D' and
needs to be determined by the calling routine.

=item $status = -1

Can not decide on split between RA and Dec i.e.
...
sub parse_coords {
    ...
    for ($i = $wid; $i <= $#words; $i ++) {
        ...
        if ($words[$i] =~ /^R(B|J|G )/i) {
            ...
            last if ($num == 6);  # Might as well stop: what to do with another number that is not an epoch?
            if ($isign == 0) {
                $ra .= "$words[$i] ";
            }
            else {
