...
sub new {
    ...
    foreach my $msb ($sp->msb) {
        ...
        foreach my $obs ($msb->unroll_obs()) {
            ...
            my $radius = Astro::PAL::DAS2R * (($height > $width) ? $height : $width) / 2;

            # Create a key so that we can check by uniqueness.
            my $id = sprintf '%.6f %.6f %.6f',
                $ra2000->radians(), $dec2000->radians(), $radius;
            ...
            if ($obs->{'instrument'} eq 'WFCAM') {
                ...
                foreach my $of ([1, 1], [1, -1], [-1, -1], [-1, 1]) {
                    my $subarray_offset = Astro::Coords::Offset->new(
                    ...
                    foreach my $cf ([-1, 1], [1, 1], [1, -1], [-1, -1]) {
                        ...
                        WORKAROUND_BOUNDS
                            && $self->_add_bounds(map {$_->radians()}
                                $corner[-1]->radec2000());
                    }

                    my $subregion = Starlink::AST::Polygon->new(
                        $skyframe,
                ...
                $region = Starlink::AST::Circle->new(
                    $skyframe,
                    1,
                    [$ra2000->radians(), $dec2000->radians()],
                    [$radius],
                    undef,
                    '');
                ...
                $region = Starlink::AST::Polygon->new(
                    $skyframe,
                    [map {my ($ra, undef) = $_->radec2000(); $ra->radians()} @corner ],
                    [map {my (undef, $dec) = $_->radec2000(); $dec->radians()} @corner],
                    undef,
...
sub write_ast {
    ...
    die 'OMP::SpRegion: type must be one of: ' . join ', ',
        keys %{$self->{'separate'}}
    ...
    if (defined $opt{'filename'}) {
        ...
        my $ch = Starlink::AST::Channel->new(sink => sub {print $fh "$_[0]\n";});
...
sub write_stcs {
    ...
    if (defined $opt{'filename'}) {
        ...
        die 'Could not open output file' unless defined $fh;
        $ch = Starlink::AST::StcsChan->new(sink => sub {print $fh "$_[0]\n";});
    }
    else {
        $ch = Starlink::AST::StcsChan->new(sink => sub {print "$_[0]\n";});
    }
...
sub get_moc {
    ...
    my $moc = Starlink::AST::Moc->new(sprintf 'MaxOrder=%i', $max_order);
...
sub _add_region {
    ...
    unless (exists $self->{'cmp'}->{$name}) {
        ...
        $self->{'cmp'}->{$name} = $self->{'cmp'}->{$name}->CmpRegion(
            $region, Starlink::AST::Region::AST__OR(), '');
    }
}
...
sub _add_bounds {
    ...
    foreach (0, 1) {
        $self->{'lbnd'}->[$_] = $l[$_]
            if (! defined $self->{'lbnd'}->[$_]
            || $l[$_] < $self->{'lbnd'}->[$_]);

        $self->{'ubnd'}->[$_] = $u[$_]
            if (! defined $self->{'ubnd'}->[$_]
            || $u[$_] > $self->{'ubnd'}->[$_]);
    }
}
...
sub _make_wcs {
    ...
    if ($bounds) {
        ...
        if ($system eq 'GAL') {
            my $galframe = Starlink::AST::SkyFrame->new('SYSTEM=GALACTIC');
