...
Include simulation configuration. Default is false.
...
sub translate {
    ...
    for my $obs (@unrolled) {
        ...
        # Add comment
        $cfg->comment(
            "Translated on " . gmtime() . "UT on host " . Net::Domain::hostfqdn()
            . " by $ENV{USER}\n"
    ...
    do {
        ...
        if (exists $info{'extra_inbeam'} and defined $info{'extra_inbeam'}) {
                ...
                and not(grep {$_ eq 'pol2_cal'} @inbeam))
            {
                ...
                @inbeam = (
                    (map {$_ ne 'pol' ? $_ : qw/pol2_wp pol2_ana/} @inbeam),
                    'pol2_cal'
                );
            }
        }
...
sub tcs_config {
    ...
    if (exists $tags{SCIENCE}
        ...
        throw OMP::Error::TranslateFail(
            "Chopped mode specified but no chop position angle defined\n")
        ...
        # offset
        my $offset = Astro::Coords::Offset->new(
            $chop_x,
            $chop_y,
            system => $chop{SYSTEM});
...
sub observing_area {
    ...
    if ($info{obs_type} eq 'skydip') {
        ...
        if ($obsmode eq 'scan') {
            ...
            @el = map {
                Astro::Coords::Angle->new($_, units => "deg")
            } ($maxel, $minel);

            $oa->skydip_velocity(
                OMP::Config->getData($self->cfgkey . ".skydip_velocity"));
        }
            ...
            if ($isskydip) {
                ...
                my $numel = OMP::Config->getData(
                    $self->cfgkey . ".skydip_numel");
                OMP::Error::FatalError->throw(
                    "Number of elevations in skydip (skydip_numel) must be at least 2 (was $numel)")
                    if $numel <= 2;
        ...
        if ($isskydip) {
            $self->output(
                $oa->skydip_mode . " skydip from $maxel to $minel degrees elevation\n");
        }
        else {
            $self->output(
        ...
        if ($pattern !~ /ellipse|daisy/i) {
            ...
            @scanpas = map {
                Astro::Coords::Angle->new($_, units => 'deg', range => "2PI")
            } @scanpas;
        }
        ...
        $oa->posang(Astro::Coords::Angle->new($refpa, units => 'deg'));
...
sub secondary_mirror {
    ...
    if ($smu->smu_mode() =~ /(jiggle_chop|chop_jiggle)/) {
        ...
        if ($maxsteps == 0) {
            ...
            my ($nsteps_total, $steps_per_off) = $self->calc_jiggle_times(
                $npts, $njigs);
...
sub _read_instrument_config {
    ...
    my $file = File::Spec->catfile(
        $self->wiredir, 'frontend', "instrument_$inst.ent");
    throw OMP::Error::FatalError(
        "$inst instrument configuration XML not found in $file !")
        unless -e $file;

    # Read it
    my $inst_cfg = JAC::OCS::Config::Instrument->new(
        File => $file,
        validation => 0,
    );
...
sub header_config {
    ...
    my $file = File::Spec->catfile(
        $self->wiredir, 'header', 'headers_' . $be . '.ent');

    my $hdr = JAC::OCS::Config::Header->new(
        validation => 0,
        File => $file
    );
    ...
    for my $i (@items) {
        ...
        if ($hdrobj->can($method)) {
            ...
            if (defined $val) {
                ...
                throw OMP::Error::FatalError(
                    "Method $method for keyword " . $i->keyword
                    . " resulted in an undefined value");
            }
        }
        else {
            throw OMP::Error::FatalError(
...
sub rts_config {
    ...
    my $file = File::Spec->catfile($self->wiredir, 'rts', $root . ".xml");
    throw OMP::Error::TranslateFail("Unable to find RTS wiring file $file")
        unless -e $file;

    my $rts = JAC::OCS::Config::RTS->new(
        File => $file,
        validation => 0
    );
...
sub pol_config {
    ...
    if (exists $info{waveplate}) {
        ...
        my $nsteps = OMP::Config->getData(
            $self->cfgkey . '.steps_per_cycle_pol');

        my $speed = 1.0 / ($nsteps * $step_time);

        $pol->spin_speed($speed);
        $self->output(
            "\t$nsteps spectra per cycle\n",
...
sub observing_mode {
    ...
    if ($otmode eq 'SpIterRawXmlObs') {
        ...
        $info->{$_} = 'raw'
            foreach qw/obs_type switching_mode mapping_mode observing_mode/;
        $info->{'inbeam'} = [];
        return;
    }
...
sub translate_scan_pattern {
    ...
    OMP::Error::SpBadStructure->throw(
        "Unrecognized OT scan pattern: '$otpatt'");
}
...
sub jig_info {
    ...
    # and store them
    $jig->posang(Astro::Coords::Angle->new($jpa, units => 'deg'));
    $jig->system($jsys);
...
sub tracking_receptor_or_subarray {
    ...
    my @configs = OMP::Config->getData(
        "jcmt_translator.tracking_receptors_or_subarrays");
...
sub calc_receptor_or_subarray_mask {
    ...
    throw OMP::Error::FatalError(
        'no frontend or similar setup component is available')
...
sub _read_file {
    ...
    my $file = shift;
    open(my $fh, '<', $file)
        or throw OMP::Error::FatalError("Unable to open file $file: $!");

    local $/ = undef;
    my $str = <$fh>;

    close($fh)
        or throw OMP::Error::FatalError("Unable to close file $file: $!");
