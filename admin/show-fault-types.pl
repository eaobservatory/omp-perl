...
GetOptions(
    ...
    'start' => sub {$match_start = 1; $match_end = $match_word = $match_any = 0;},
    'end' => sub {$match_end = 1; $match_start = $match_word = $match_any = 0;},
    'word' => sub {$match_word = 1; $match_start = $match_end = $match_any = 0;},
...
sub show {
    ...
    for my $c (sort keys %{$cats}) {
        ...
        printf "\n%s: %s\n", term_bold('Category'), term_reverse(term_bold($c));
...
sub make_system {
    ...
    return _make_type_or_sys(
        $sys,
        OMP::Fault->faultSystems($cat, include_hidden => $hidden));
}
...
BEGIN {
    my ($norm, $bold, $under, $reverse) = map "\e[${_}m", 0, 1, 4, 7;

    sub term_bold {
        return map {join '', $bold, $_, $norm} @_;
    }

    sub term_underline {
        return map {join '', $under, $_, $norm} @_;
    }

    sub term_reverse {
        return map {join '', $reverse, $_, $norm} @_;
    }
}
