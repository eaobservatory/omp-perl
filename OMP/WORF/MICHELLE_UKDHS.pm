package OMP::WORF::MICHELLE_UKDHS;

=head1 NAME

OMP::WORF::MICHELLE_UKDHS - MICHELLE_UKDHS (Michelle with ORAC DHS) specific
functions for WORF
...
use OMP::WORF::MICHELLE_UKDHS;

  my $worf = new OMP::WORF::MICHELLE_UKDHS( obs => $obs, suffix => $suffix );
...
This subclass of C<OMP::WORF> supplies Michelle-specific functions
for WORF. In particular, it allows for plotting of images and retrieving
a list of valid suffices for Michelle data. This subclass is meant for
Michelle data taken with the ORAC data handling system.
...
The valid suffices for a Michelle observation.

  @suffices = $worf->suffices( $group );

This method returns a list of valid suffices for reduced Michelle data, as
...
sub suffices {
  ...
  if( $group ) {
    @suffices = qw/ _mos _dbs _fc /;
  } else {
    @suffices = qw/ _ff _ss _wce /;
  }
...
sub get_filename {
  ...
  if( $group ) {
    ...
    $filename = "m" . $ut . "_" . sprintf("%05d", $runnr) . $self->suffix;
    ...
    $filename = "m" . $ut . "_" . sprintf("%05d", $runnr);
