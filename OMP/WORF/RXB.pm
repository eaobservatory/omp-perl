package OMP::WORF::RXB;

=head1 NAME

OMP::WORF::RXB - RXB-specific functions for WORF
...
use OMP::WORF::RXB;
  ...
  my $worf = new OMP::WORF::RXB( obs => $obs, suffix => $suffix );
...
This subclass of C<OMP::WORF> supplies RXB-specific functions
for WORF. In particular, it allows for plotting of images and retrieving
a list of valid suffices for RXB data.
...
The valid suffices for an RXB observation.

  @suffices = $worf->suffices( $group );

This method returns a list of valid suffices for reduced RXB data, as
