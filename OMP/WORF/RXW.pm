package OMP::WORF::RXW;

=head1 NAME

OMP::WORF::RXW - RXW-specific functions for WORF
...
use OMP::WORF::RXW;
  ...
  my $worf = new OMP::WORF::RXW( obs => $obs, suffix => $suffix );
...
This subclass of C<OMP::WORF> supplies RXW-specific functions
for WORF. In particular, it allows for plotting of images and retrieving
a list of valid suffices for RXW data.
...
The valid suffices for an RXW observation.

  @suffices = $worf->suffices( $group );

This method returns a list of valid suffices for reduced RXW data, as
