package OMP::CGI::ProjectPage;

=head1 NAME

OMP::CGI::ProjectPage - Web display of complete project information pages
...
sub fb_proj_summary {
  ...
  # Observed MSB table
  OMP::CGI::MSB::fb_msb_observed($q, $cookie{projectid});

  print $q->hr;
  print $q->h2("MSBs to be observed");

  # MSBs to be observed table
  OMP::CGI::MSB::fb_msb_active($q, $cookie{projectid});
...
sub list_projects {
  ...
  OMP::CGI::Project::list_projects_form($q);
...
sub list_projects_output {
  ...
  if (@$projects) {
	...
	OMP::CGI::Project::proj_sum_table(\@sorted, $q);
      } else {
	# Display table with semester and country headings
	# since we sorted by project ID
	OMP::CGI::Project::proj_sum_table(\@sorted, $q, 1);
      }
	...
	OMP::CGI::Project::proj_status_table($q, %details);
	
	print $q->h3('MSBs observed');
	OMP::CGI::Feedback::fb_msb_observed($q, $project->projectid);
	
	print $q->h3('MSBs to be observed');
	OMP::CGI::Feedback::fb_msb_active($q,$project->projectid);	
      }
...
C<OMP::CGI::Project>, C<OMP::CGI::Feedback>, C<OMP::CGI::MSB>
