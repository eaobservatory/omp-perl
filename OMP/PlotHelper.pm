...
sub bin_up {
  ...
  throw OMP::Error::BadArgs("Argument 'size' must be greater than 0")
    unless ($size > 0);
  ...
  my $high_val = $low_val + $size;

  # Bin up the values
  my @result;
  my $bin_num = 0; # Keep track of bin number
  my $count = 0; # Keep count of values added to the bin
  for my $val (@values) {

    until ($val->[0] >= $low_val and $val->[0] <= $high_val) {

      # Value was outside the range of this bin.  Keep moving
      # through the bins until we find one that the value belongs in
      $result[$bin_num] = 0
	unless (defined $result[$bin_num]); # Set bin value to zero if nothing
                                            # went in this bin.
      $bin_num++; # Move to the next bin
      $low_val += $size;
      $high_val += $size;
      $count = 0; # Reset the bin value counter
    };

    # Found a bin that the value belongs
    # Add the y value to the bin
    $result[$bin_num] += $val->[1];
    $count++;

    # See if next value can go in this bin too
  }
  return @result;
}
